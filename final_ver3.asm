...
KEYBOARD:
	...
	and 	$at, $t1, $t2
	beq	$at, $t2, CHECK_TYPE	
	nop              
CHECK_TYPE:
	beq	$t5, 1, PUT_STRING	# if interrupt is from keyboard
	nop 
...
PUT_STRING:
READ_KEY:  lb   $t0, 0($k0)            	# $t0 = [$k0] = KEY_CODE. Here we use lb for efficient data storage because ASCII only goes to 127 max, which is 8 bits
#WAIT_FOR_DIS: 
...
SHOW_KEY: 
	     sb $t0, 0($s0)              	# show the input key when key ready has a signal 1 on MMIO
             ...
             la  $s5,storestring		# lay $t7 lam dia chi co so cua chuoi nhap vao
           					#this below part will check if the user hits enter or not
             add $s5,$s5,$s4		
